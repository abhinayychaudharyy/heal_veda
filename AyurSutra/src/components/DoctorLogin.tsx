  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/login/${encodeURIComponent(formData.username)}/${encodeURIComponent(formData.password)}`,
        { method: 'GET' }
      );

      const textResponse = await response.text();

      if (response.ok && textResponse === 'User present') {
        onLogin({
          id: 'doc-1', // Default ID since backend doesn't return one
          name: 'Dr. ' + formData.username.replace(/^Dr\.\s*/i, ''),
          email: formData.username + '@healveda.com',
          userType: 'doctor'
        });
      } else {
        alert("Invalid credentials or doctor not found.");
      }
    } catch (error) {
          ...
          <CardContent className="space-y-6">
            ...
            <div className="text-center pt-4 border-t border-gray-700">
              <p className="text-sm text-gray-500 mb-2">
                Restricted access for authorized medical practitioners only.
              </p>
            </div>
          </CardContent>
        </Card>
