...
export function PatientRegister({ onNavigate }: PatientRegisterProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend returns string message. Check if it contains "Error" or "exists"
      // because it returns 200 OK even for business logic errors.
      const msg = typeof response.data === 'string' ? response.data : JSON.stringify(response.data);

      if (msg.toLowerCase().includes("error") || msg.toLowerCase().includes("exist")) {
        throw new Error(msg);
      }
      ...
      const errorMsg = error.message || error.response?.data?.message || "Registration failed. Try again.";
      alert(errorMsg);
    } finally {
      setLoading(false);
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(value: string) => handleSelectChange('gender', value)} required>
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={(value: string) => handleSelectChange('city', value)} required>
                    <SelectTrigger className="bg-gray-700/50 border-gray-600 text-white focus:border-green-400 focus:ring-green-400">
                      <SelectValue placeholder="Select city" />
                    </SelectTrigger>
                    <SelectContent className="bg-gray-800 border-gray-600">
                      <SelectItem value="pune" className="text-white hover:bg-gray-700">New Delhi</SelectItem>
                      <SelectItem value="mumbai" className="text-white hover:bg-gray-700">Greater Noida</SelectItem>
                      <SelectItem value="kolhapur" className="text-white hover:bg-gray-700">Jaipur</SelectItem>
                      <SelectItem value="sangli" className="text-white hover:bg-gray-700">Mumbai</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
